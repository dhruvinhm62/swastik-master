...
class _AllCategoryScreenState extends State<AllCategoryScreen> {
                                      ...
                                      return Slidable(
                                        endActionPane: ActionPane(
                                          ...
                                          children: [
                                            ...
                                            SlidableAction(
                                              onPressed: (context) async {
                                                await deleteCategory(context, index, controller);
                                              },
  ...
  Future<void> deleteCategory(BuildContext context, int index, CategoryController controller) async {
    await Future.delayed(const Duration(milliseconds: 100)).then(
      (value) async {
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: GestureDetector(
                            onTap: () async {
                              String name = controller.categories[index].name ?? "";

                              Navigator.pop(c1);
                              await categoryController.deleteCategory(controller.categories[index].id ?? "").then(
                                (value) {
                                  if (value) {
                                    if (!context.mounted) return;
                                    AppSnackbar.snackbar(context, "$name category deleted.");
                                  } else {
                                    if (!context.mounted) return;
